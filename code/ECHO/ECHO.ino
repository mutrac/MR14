...
void setup() {
  ...
  /* --- LCD --- */
  Serial3.print("BOOTING..."); delay(MEDIUM); clear();
  ...
  while (nokill()) {
    ...
    if (Serial2.available()) {
      ...
      // Else if key is bad, display error message.
        ...
        Serial3.print("INCORRECT KEY"); delay(SHORT); clear();
      }
    }
  }
}
...
void standby() {
  ...
  while (nokill()) {
    ...
    Serial3.print("STANDBY"); delay(SHORT); clear();
...
void ignition() {
  ...
  while (start() && nolock() && nokill()) {
    ...
    Serial3.print("IGNITION"); delay(SHORTER); clear();
  }
}
...
void run() {
  ...
  Serial3.print("RUNNING"); delay(MEDIUM); clear();
  ...
  while (nokill()) {
    ...
    // Display to LCD
    Serial3.print(BUFFER_FUEL); newline();
...
boolean nokill() {
  ...
  if (KILL_SEAT) {
    ...
    if (digitalRead(KILL_SEAT_PIN)) {
      ...
      Serial3.print("DRIVER NOT ON SEAT"); delay(SHORT); clear();
      return false;
    }
    else {
      return true;
  ...
  // If hitch is removed
    ...
    Serial3.print("HITCH DETACHED"); delay(SHORT); clear();
    return false;
  }
  // If Kill button is pressed
    ...
    Serial3.print("KILL BUTTON PRESSED"); delay(SHORT); clear();
    return false;
  }
...
boolean nolock() {
  ...
  if (LOCK_LEFTBRAKE || LOCK_RIGHTBRAKE) {
    ...
    Serial3.print("BRAKES NOT ENGAGED"); delay(SHORT); clear();
  }
  // Get CVT state
...
int ballast() {
  ...
  if (!MOTOR_FAULT) {
    ...
    if (LIMIT_NEAR || LIMIT_FAR) {
      ...
      Serial3.print("LIMIT REACHED"); delay(SHORT); clear();
      INTERVAL += SHORT;
      MOTOR_SPEED = OFF;
    }
    else {
  ...
  // If motor fault, disable motor.
    ...
    Serial3.print("MOTOR FAULT"); delay(SHORT); clear();
    INTERVAL += SHORT;
    MOTOR_SPEED = OFF;
  }
